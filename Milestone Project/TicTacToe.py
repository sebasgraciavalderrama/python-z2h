...
def tie(number_of_plays):
    # Check if at any given point in time the result of the match is a tie, returns True if tie
    if number_of_plays >= 9:
        print('Match has resulted with a tie!')
        return True
    return False

def win(tictactoe_board, player=''):
    # Check if any player won after inserting a 'X' or 'O' in
    # the board, returns True if any player has won
...
def get_index_of_player_choice(tictactoe_board, player_choice):
    # Returns the position x, y (tuple) of the player's choice,
    # if not found or found symbol ('X', 'Y') returns (-1,-1)
...
def print_board(tictactoe_board):
    # Prints board after any update (instert, win, etc)
    print("#---------------------------------------#")
